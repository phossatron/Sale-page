...
/* ============================================================
   เผยแพร่ขึ้นเว็บจริง (commit js/content.js เข้า GitHub โดยตรง)
   ใช้ได้จากทุกอุปกรณ์ รวมถึงมือถือ — ไม่ต้องใช้ Terminal
   ต้องใส่ GitHub token ครั้งแรกครั้งเดียว (เก็บไว้ในเบราว์เซอร์เครื่องนั้น)
   ============================================================ */
var PUBLISH = {
  owner: 'phossatron',
  repo: 'Sale-page',
...
function contentFileSource() {
  return '/* ============================================================\n' +
         '   content.js — all page content in one place\n' +
         '   Published from the on-page editor on ' + new Date().toLocaleString('th-TH') + '\n' +
         '   ============================================================ */\n' +
...
function ghApi(path, opts) {
  opts = opts || {};
  return fetch('https://api.github.com/repos/' + PUBLISH.owner + '/' + PUBLISH.repo + '/' + path, {
...
function ghPutFile(path, text, message) {
  return ghFileSha(path)
    .then(function (sha) {
      ...
      if (sha) body.sha = sha;
      return ghApi('contents/' + path, { method: 'PUT', body: body });
    });
}
...
function publishToWeb() {
  ...
  if (!confirm('เผยแพร่เนื้อหาปัจจุบันขึ้นเว็บจริง?\n\n' +
      ...
      'เว็บจะเปลี่ยนตามภายใน 1–2 นาที')) return;

  var msg = 'Update content from the on-page editor';
  var el = document.getElementById('btn-publish');
  var old = el.textContent;
  el.disabled = true;
  el.textContent = '⏳ กำลังเผยแพร่...';
  flash('กำลังส่งข้อมูลขึ้น GitHub...');

  // เขียนทีละไฟล์ตามลำดับ (กันชนกันเองใน branch เดียว)
  PUBLISH.paths.reduce(function (chain, path, i) {
    return chain.then(function () {
      el.textContent = '⏳ ' + (i + 1) + '/' + PUBLISH.paths.length + '...';
      return ghPutFile(path, code, msg);
    });
  }, Promise.resolve())
  .then(function () {
    el.disabled = false; el.textContent = old;
    flash('เผยแพร่แล้ว! เว็บจะอัปเดตใน 1–2 นาที');
    setTimeout(function () {
      if (confirm('เผยแพร่สำเร็จ ✅\n\nเปิดเว็บจริงเพื่อตรวจสอบเลยไหม?\n(อาจต้องรอ 1–2 นาที แล้วรีเฟรช)')) {
        window.open(PUBLISH.site, '_blank');
      }
    }, 300);
  })
  .catch(function (err) {
    ...
    if (/Bad credentials|401/i.test(m)) { setGhToken(''); m += '\n\nToken ใช้ไม่ได้แล้ว — กรุณาใส่ใหม่'; }
    if (/Not Found|404/i.test(m)) m += '\n\n(มักเกิดจาก token ไม่มีสิทธิ์ Contents: Read and write ของ repo นี้)';
    alert('เผยแพร่ไม่สำเร็จ\n\n' + m);
    flash('เผยแพร่ไม่สำเร็จ', true);
  });
}
...
function askToken() {
  ...
  m.innerHTML =
    '<div class="modal-card">' +
      '<h3>เชื่อมต่อ GitHub เพื่อเผยแพร่</h3>' +
      '<p class="hint">ใส่ GitHub token ครั้งเดียว แล้วจะกดเผยแพร่จากอุปกรณ์นี้ได้ตลอด<br>' +
        'ระบบจะเก็บ token ไว้ในเบราว์เซอร์เครื่องนี้เท่านั้น ไม่ส่งไปที่อื่นนอกจาก api.github.com</p>' +
      '<ol class="hint steps">' +
        '<li>เปิด <b>github.com/settings/personal-access-tokens/new</b></li>' +
        '<li>Repository access → <b>Only select repositories</b> → เลือก <b>' + PUBLISH.repo + '</b></li>' +
        '<li>Permissions → Repository permissions → <b>Contents</b> = <b>Read and write</b></li>' +
        '<li>กด Generate token แล้วคัดลอกมาวางด้านล่าง</li>' +
      '</ol>' +
      '<label class="field"><span>GitHub token</span>' +
        '<input id="m-token" type="password" placeholder="github_pat_… หรือ ghp_…" autocomplete="off"></label>' +
      '<div class="modal-act">' +
        (ghToken() ? '<button class="btn ghost sm" id="m-forget" type="button">ลบ token ออกจากเครื่องนี้</button>' : '') +
        '<span class="spacer"></span>' +
        '<button class="btn ghost" id="m-cancel" type="button">ยกเลิก</button>' +
        '<button class="btn primary" id="m-ok" type="button">บันทึกแล้วเผยแพร่</button>' +
      '</div>' +
    '</div>';
  m.classList.add('open');
  m.querySelector('#m-cancel').onclick = closeModal;
  var forget = m.querySelector('#m-forget');
  if (forget) forget.onclick = function () { setGhToken(''); closeModal(); flash('ลบ token แล้ว'); };
  m.querySelector('#m-ok').onclick = function () {
    var v = m.querySelector('#m-token').value.trim();
    if (!v) return alert('กรุณาใส่ token');
    setGhToken(v);
    closeModal();
    publishToWeb();
  };
...
function initToolbar() {
  ...
  document.getElementById('btn-publish').onclick = publishToWeb;
